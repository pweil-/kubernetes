...
func (s *serviceAccount) Admit(a admission.Attributes) (err error) {
	...
	if err != nil {
		admission.NewForbidden(a, fmt.Errorf("Error looking up service account token for %s/%s: %v", a.GetNamespace(), pod.Spec.ServiceAccount, err))
	}
	...
	for _, volume := range pod.Spec.Volumes {
		...
		if volume.Secret != nil && volume.Secret.SecretName == serviceAccountToken {
			serviceAccountTokenVolume = volume.Name
			break
		}
	}

	// Create the volume and volume name for the ServiceAccountTokenSecret
	if len(serviceAccountTokenVolume) == 0 {
		// Try naming the volume the same as the serviceAccountToken, and uniquify if needed
		serviceAccountTokenVolume = serviceAccountToken
		if allVolumeNames.Has(serviceAccountTokenVolume) {
			serviceAccountTokenVolume = api.SimpleNameGenerator.GenerateName(fmt.Sprintf("%s-", serviceAccountToken))
		}
...
func (s *serviceAccount) getServiceAccountTokens(serviceAccount *api.ServiceAccount) ([]*api.Secret, error) {
	...
	for _, obj := range index {
		...
		name, _ := token.Annotations[api.ServiceAccountNameKey]
		uid, _ := token.Annotations[api.ServiceAccountUIDKey]
