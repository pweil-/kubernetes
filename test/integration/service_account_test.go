...
func getReferencedServiceAccountToken(c *client.Client, ns string, name string, shouldWait bool) (string, string, error) {
	...
	findToken := func() (bool, error) {
		...
		for _, ref := range user.Secrets {
			...
			if secret.Type != api.SecretTypeServiceAccountToken || secret.Data == nil || secret.Annotations == nil {
				continue
			}
			name, _ := secret.Annotations[api.ServiceAccountNameKey]
			uid, _ := secret.Annotations[api.ServiceAccountUIDKey]
			if name == user.Name && uid == string(user.UID) {
				tokenName = secret.Name
				token = string(secret.Data[api.ServiceAccountTokenKey])
				return true, nil
			}
		}
