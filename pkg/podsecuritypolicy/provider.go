...
import (
	...
	"k8s.io/kubernetes/pkg/podsecuritypolicy/selinux"
	"k8s.io/kubernetes/pkg/podsecuritypolicy/user"
	"k8s.io/kubernetes/pkg/util/fielderrors"
)

// simpleProvider is the default implementation of PodSecurityPolicyProvider
type simpleProvider struct {
	psp               *experimental.PodSecurityPolicy
...
func NewSimpleProvider(psp *experimental.PodSecurityPolicy) (PodSecurityPolicyProvider, error) {
	if psp == nil {
	...
	case experimental.RunAsUserStrategyMustRunAs:
		userStrat, err = user.NewMustRunAs(&psp.Spec.RunAsUser)
	case experimental.RunAsUserStrategyMustRunAsRange:
		userStrat, err = user.NewMustRunAsRange(&psp.Spec.RunAsUser)
	case experimental.RunAsUserStrategyMustRunAsNonRoot:
		userStrat, err = user.NewRunAsNonRoot(&psp.Spec.RunAsUser)
	case experimental.RunAsUserStrategyRunAsAny:
	...
	case experimental.SELinuxStrategyMustRunAs:
		seLinuxStrat, err = selinux.NewMustRunAs(&psp.Spec.SELinuxContext)
	case experimental.SELinuxStrategyRunAsAny:
...
func (s *simpleProvider) CreateSecurityContext(pod *api.Pod, container *api.Container) (*api.SecurityContext, error) {
	...
	if s.psp.Spec.RunAsUser.Type == experimental.RunAsUserStrategyMustRunAsNonRoot {
