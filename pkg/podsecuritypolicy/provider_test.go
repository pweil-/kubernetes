...
func TestCreateSecurityContextNonmutating(t *testing.T) {
	...
	createSCC := func() *experimental.PodSecurityPolicy {
		var uid int64 = 1
...
func TestValidateFailures(t *testing.T) {
	defaultSCC := func() *experimental.PodSecurityPolicy {
		return &experimental.PodSecurityPolicy{
			ObjectMeta: api.ObjectMeta{
				Name: "scc-sa",
			},
	...
	var uid int64 = 999
	var badUID int64 = 1
	failUserSCC.Spec.RunAsUser = experimental.RunAsUserStrategyOptions{
		Type: experimental.RunAsUserStrategyMustRunAs,
		UID:  &uid,
	}
	...
	failSELinuxSCC.Spec.SELinuxContext = experimental.SELinuxContextStrategyOptions{
		Type: experimental.SELinuxStrategyMustRunAs,
...
func TestValidateSuccess(t *testing.T) {
	...
	userSCC := defaultSCC()
	var uid int64 = 999
	userSCC.Spec.RunAsUser = experimental.RunAsUserStrategyOptions{
		Type: experimental.RunAsUserStrategyMustRunAs,
		UID:  &uid,
	}
	userPod := defaultPod()
	...
	seLinuxSCC.Spec.SELinuxContext = experimental.SELinuxContextStrategyOptions{
		Type: experimental.SELinuxStrategyMustRunAs,
		SELinuxOptions: &api.SELinuxOptions{
			Level: "foo",
		},
	...
	privSCC := defaultSCC()
	privSCC.Spec.Privileged = true
	privPod := defaultPod()
	var priv bool = true
	privPod.Spec.Containers[0].SecurityContext.Privileged = &priv

	capsSCC := defaultSCC()
	capsSCC.Spec.Capabilities = []api.Capability{"foo"}
	capsPod := defaultPod()
