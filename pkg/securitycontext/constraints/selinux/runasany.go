...
// pod's context or an empty SELinuxOptions if none is set.
type runAsAny struct{}

// NewRunAsAny creates a new runAsAny strategy.
func NewRunAsAny() SELinuxSecurityContextConstraintsStrategy {
	return &runAsAny{}
}

// Generate creates the SELinuxOptions based on policy rules.
func (s *runAsAny) Generate(podSecurityContext *api.SecurityContext) *api.SELinuxOptions {
	if podSecurityContext != nil && podSecurityContext.SELinuxOptions != nil {
