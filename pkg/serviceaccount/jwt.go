...
const (
	...
	SubjectClaim            = "sub"
	IssuerClaim             = "iss"
	ServiceAccountNameClaim = "http://kubernetes.io/serviceaccount/serviceAccountName"
	ServiceAccountUIDClaim  = "http://kubernetes.io/serviceaccount/serviceAccountUID"
	SecretNameClaim         = "http://kubernetes.io/serviceaccount/secretName"
	NamespaceClaim          = "http://kubernetes.io/serviceaccount/namespace"
)
...
func (j *jwtTokenAuthenticator) AuthenticateToken(token string) (user.Info, bool, error) {
	...
	for _, key := range j.keys {
		...
		if j.lookup {
			...
			if err != nil || secret.Data == nil || len(secret.Data[api.ServiceAccountTokenKey]) == 0 {
